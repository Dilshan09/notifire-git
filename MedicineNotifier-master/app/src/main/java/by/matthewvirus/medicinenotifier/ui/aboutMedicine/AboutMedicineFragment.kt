    ...
    HomeActivity.Callbacks
{
    ...
    // Update the user interface with medicine data
    ...
    // Set up the adapter for the Spinner that represents medicine times per day
    ...
    // Create a Medicine object with updated data from UI elements
    ...
    // Set up the click listener for the Update Medicine button
    ...
    // Set up the click listener for the Take Medicine button
    ...
    // Pause the medicine notification
    @RequiresApi(Build.VERSION_CODES.S)
    ...
    // Continue the medicine notification
    @RequiresApi(Build.VERSION_CODES.S)
    ...
    // Retrieve the index of the medicine from parent fragment arguments
    private fun getIndexFromParentFragment() {
        index = arguments?.getInt(INDEX_ARGUMENT)
    }

    // Return to the HomeActivity by popping the current fragment from the back stack
    private fun returnToHomeActivity() {
        activity?.supportFragmentManager?.popBackStack()
    }

    // Set up the Spinner with selected value
